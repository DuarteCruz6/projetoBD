...
def show_3_flights_with_tickets(partida, chegada):
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            flights = cur.execute(
                """
                SELECT voo.no_serie, voo.partida, voo.chegada, voo.hora_partida
                FROM voo
...
def getSeat(cur,no_serie,prim_classe):
    cur.execute(
        """
        ...
        AND NOT EXISTS (
            SELECT 1
            FROM bilhete
            WHERE bilhete.no_serie = assento.no_serie AND bilhete.lugar = assento.lugar
        )
        LIMIT 1
        """,
...
def createTickets(cur, voo_id, pairs, nif_cliente, tempo_voo):
    ...
    for pair in pairs:
        ...
        # pair = (nome_passageiro, classe de bilhete)
        #creates <bilhete>
        if pair[1]: # This is the line where your error is happening
            # ... your logic for when the boolean is True ...
            print(f"DEBUG: {pair[0]} has boolean True.")
        else:
            # ... your logic for when the boolean is False ...
            print(f"DEBUG: {pair[0]} has boolean False.")
        ...
        if prim_classe:
            ...
            preco = 40 * tempo_voo
        lugar = getSeat(cur, no_serie, prim_classe)
        
        if lugar is None:
            return False
        cur.execute(
            """
                INSERT INTO bilhete (id, voo_id, codigo_reserva, nome_passageiro, preco, prim_classe, lugar, no_serie)
                VALUES (
                    %(id)s,%(voo_id)s,%(codigo_reserva)s,%(nome_passageiro)s,%(preco)s,
                        %(prim_classe)s,%(lugar)s,%(no_serie)s
                )
            """,
            {
                "id":last_bilhete_id+offSet, "voo_id":voo_id, "codigo_reserva":codigo_reserva, 
                "nome_passageiro":nome_passageiro, "preco":preco, "prim_classe":prim_classe,
                "lugar":lugar,"no_serie":no_serie,
            }
        )
        offSet+=1
    return True
...
def calculateTempoVoo(cur, voo_id):
    ...
    if row:
        ...
        partida_dt = datetime.combine(today, partida)
        chegada_dt = datetime.combine(today, chegada)
...
def buyTickets(voo_id):
    ...
    try:
        with pool.connection() as conn:
            with conn.cursor() as cur:
                ...
                if not createTickets(cur, voo_id, pairs, nif_cliente, tempo_voo):
    ...
    return jsonify({"message": "Bilhetes comprados com sucesso."}), 200


@app.route(
    "/checkin/<bilhete_id>/",methods=("POST",),
)
def do_checkIn_ticket(bilhete_id):
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                    SELECT no_serie, prim_classe, lugar FROM bilhete
            ...
            no_serie, prim_classe, lugar_atual = row
